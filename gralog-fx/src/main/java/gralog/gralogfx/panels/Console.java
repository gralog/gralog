        ...
        if (parser.getDialogState() == DONE){
            switch (parser.getDialogAction()) {
                ...
                case CONNECT_CLIQUE:            dialog.connectClique(parameters,currentPane.getStructure());
                                                break;
                case CONNECT_BICLIQUE:          dialog.connectBiclique(parameters);
                                                break;
                case CONNECT_SELFLOOP:          dialog.connectSelfloop(parameters);
                                                break;
                case CONNECT_PATH:              dialog.connectPath(parameters);
                                                break;
                case CONNECT_CYCLE:             dialog.connectCycle(parameters);
                                                break;
                case CONNECT_MATCHING:          dialog.connectMatching(parameters);
                                                break;
                case CONNECT_FORMULA:           dialog.connectFormula(parameters);
                                                break;
                case CONNECT_2_LISTS_FORMULA:   dialog.connect2ListsFormula(parameters);
                                                break;
